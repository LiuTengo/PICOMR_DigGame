...
public class ToolManager : MonoBehaviour
{
    ...
    [SerializeField]private SwitchableObject[] Tools;
    private SwitchableObject currentTool;
    ...
    private void Start()
    {
        if (Tools.Length>0)
        {
            ...
            {
                tool?.OnReleaseThisObject();
            }
            
            currentToolIndex = 0;
            currentTool = Tools[currentToolIndex];
            currentTool?.OnUseThisObject();
        }
        else
        {
            Debug.LogError("Tools is empty!");
        }
    }
    ...
    {
        currentTool?.OnReleaseThisObject();
        currentToolIndex = (currentToolIndex+1)%Tools.Length;
        currentTool = Tools[currentToolIndex];
        currentTool?.OnUseThisObject();
    }
    
    public void SwitchToPrimaryObject()
    {
        currentTool?.OnReleaseThisObject();
        currentToolIndex = (Tools.Length + currentToolIndex-1)%Tools.Length;
        currentTool = Tools[currentToolIndex];
        currentTool?.OnUseThisObject();
    }
